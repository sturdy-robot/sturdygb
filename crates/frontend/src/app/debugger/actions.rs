...
impl EmuApp {
    ...
    pub(in crate::app) fn toggle_debugger_pause(&mut self) {
        if self.is_paused() {
            self.debugger
                .prepare_resume(self.runtime.loaded_game.as_ref());
        }
    ...
    pub(in crate::app) fn request_debugger_step(&mut self, step: DebuggerStepKind) {
        match step {
            DebuggerStepKind::Over => self
                .debugger
                .request_step_over(self.runtime.loaded_game.as_ref()),
            DebuggerStepKind::Into => self
                .debugger
                .request_step_into(self.runtime.loaded_game.as_ref()),
            DebuggerStepKind::Out => self
                .debugger
                .request_step_out(self.runtime.loaded_game.as_ref()),
        }
        self.debugger.open();
